...
def source
  ...
  def vendor
    @vendor_id = params['vendor']
     @more_info=ReferenceDatum.where("id= ?", @vendor_id)
  ...
  # if session[:user_id]
    ...
    #@mm = User.where("id = ?", 4).select( "user_fname, user_lname")
      @mm =Term_dn.where("concept_ID = ? ", @esci )
     # @def =DefinitonDn.where("concept_ID = ? ", @esci )
...
#Force Given 
    ...
    @db_morersfl = SourceDatum.where(["`FORCE_NAME` LIKE ?", '%RSFL%']).count
    @db_moreairforce = SourceDatum.where(["`FORCE_NAME` LIKE ?", '%AIR FORCE%']).count
    # @db_force_others = db_morecommon-

# Force Available
    @db_morecmm_avil = SourceDatum.find_by_sql(["SELECT count(*) as tot  FROM `source_data` where ECCMA_valid='s'"])
    @db_morecmm_common=SourceDatum.where(["ECCMA_valid = ? and `FORCE_NAME` LIKE ?", 's','%COMMON%']).count      
    @db_morecmm_msd = SourceDatum.where(["ECCMA_valid = ? and `FORCE_NAME` LIKE ?", 's','%MSD%']).count 
    @db_morecmm_rsadf =SourceDatum.where(["ECCMA_valid = ? and `FORCE_NAME` LIKE ?", 's','%RSADF%']).count 
    @db_morecmm_rsaf = SourceDatum.where(["ECCMA_valid = ? and `FORCE_NAME` LIKE ?", 's','%RSAF%']).count 
    @db_morecmm_rsdf = SourceDatum.where(["ECCMA_valid = ? and `FORCE_NAME` LIKE ?", 's','%RSDF%']).count 
    @db_morecmm_rslf = SourceDatum.where(["ECCMA_valid = ? and `FORCE_NAME` LIKE ?", 's','%RSLF%']).count 
    @db_morecmm_rsnf = SourceDatum.where(["ECCMA_valid = ? and `FORCE_NAME` LIKE ?", 's','%RSNF%']).count 
    @db_morecmm_rslf = SourceDatum.where(["ECCMA_valid = ? and `FORCE_NAME` LIKE ?", 's','%RSFL%']).count 
    @db_morecmm_airforce = SourceDatum.where(["ECCMA_valid = ? and `FORCE_NAME` LIKE ?", 's','%AIR FORCE%']).count 
...
#Country Given
    ...
    @more_countrycode_1 = SourceDatum.find_by_sql(["SELECT distinct COUNTRY_ECCMA, count(*) as total FROM `source_data` where COUNTRY_ECCMA!='' group by COUNTRY_ECCMA order by total DESC limit 0,1"])
    ...
    @more_countrycode_2 = SourceDatum.find_by_sql(["SELECT distinct COUNTRY_ECCMA, count(*) as total FROM `source_data` where COUNTRY_ECCMA!='' group by COUNTRY_ECCMA order by total DESC limit 1,1"])
    @more_countrycode_3 = SourceDatum.find_by_sql(["SELECT distinct COUNTRY_ECCMA, count(*) as total FROM `source_data` where COUNTRY_ECCMA!='' group by COUNTRY_ECCMA order by total DESC limit 2,1"])
    @more_countrycode_4 = SourceDatum.find_by_sql(["SELECT distinct COUNTRY_ECCMA, count(*) as total FROM `source_data` where COUNTRY_ECCMA!='' group by COUNTRY_ECCMA order by total DESC limit 3,1"])
    @more_countrycode_5 = SourceDatum.find_by_sql(["SELECT distinct COUNTRY_ECCMA, count(*) as total FROM `source_data` where COUNTRY_ECCMA!=''  group by COUNTRY_ECCMA order by total DESC limit 4,1"])

#Country Available
    @more_Availcountry = SourceDatum.find_by_sql(["SELECT COUNTRY_ECCMA,count(*) As total FROM `source_data` where ECCMA_valid='s'"])
...
def editcleansed
    ...
    @technical=CmmTechnicalSpecification.select("property","value","source","value_old").where("cat_id = ? ",@id)
    @reference=CmmRxmlReference.select("property","value","value2,ECCMA_Ests,value_old").where("cat_id = ?",@id)
end
  ...
  def showmap
    ...
    @factory=MmFactory.where("DISTRICT = ? and Language = ? ",@district,language).order('Factory_Name ASC')
  end

  def factory_details
   @factid = params["factid"]
    @factory_details=FactoryMap.where("FACTORY_ID = ? ",@factid)
    @manf_details=Classification.select("DISTINCT(catalog_name)").where("FACTORY_ID = ? ",@factid).limit('0,5').uniq
    @count=Classification.select("DISTINCT(catalog_name)").where("FACTORY_ID = ? ",@factid).limit('0,5').uniq.count
    respond_to do |format|
